...
public class BishopMenu extends MenuBuilder {

    public BishopMenu(CrownArmour crownArmour, Player player) {
        super(crownArmour, player, crownArmour.getConfigFile().getString("settings.bishop.title"), crownArmour.getConfigFile().getConfig().getInt("settings.bishop.rows"));
    }

    @Override
    public BishopMenu init() {
    ...
    public ItemStack getItem(ItemStack itemStack, String id) {
        ...
        if (itemMeta != null && itemMeta.hasLore()) {
            for (String s : itemMeta.getLore()) {
                lore.add(s.replaceAll("%price%", "" + getPlugin().getConfigFile().getDouble("bishop." + id + ".price")));
            }
        }

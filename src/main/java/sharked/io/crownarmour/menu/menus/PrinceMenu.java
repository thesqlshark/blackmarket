...
public class PrinceMenu extends MenuBuilder {

    public PrinceMenu(CrownArmour crownArmour, Player player) {
        super(crownArmour, player, crownArmour.getConfigFile().getString("settings.prince.title"), crownArmour.getConfigFile().getConfig().getInt("settings.prince.rows"));
    }

    @Override
    public PrinceMenu init() {
    ...
    public ItemStack getItem(ItemStack itemStack, String id) {
        ...
        if (itemMeta != null && itemMeta.hasLore()) {
            for (String s : itemMeta.getLore()) {
                lore.add(s.replaceAll("%price%", "" + getPlugin().getConfigFile().getDouble("prince." + id + ".price")));
            }
        }
